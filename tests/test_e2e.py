...
async def test_e2e_customer_journey(client, db):
    register_resp = await client.post("/api/auth/register", json={"username": "journey", "email": "j@t.com", "password": "pass1234"})
    print(f"Register response: status={register_resp.status_code}, body={register_resp.text}")
    ...
    print(f"Create product response: status={resp.status_code}, body={resp.text}")
